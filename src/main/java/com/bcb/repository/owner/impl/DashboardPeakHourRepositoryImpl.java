...
public class DashboardPeakHourRepositoryImpl implements DashboardPeakHourRepository {
	...
	private static final String SQL_HEATMAP =
		    "SELECT " +
		    "    (DATEPART(WEEKDAY, bs.booking_date) + 5) % 7 + 1 AS day_of_week, " +
		    ...
		    "WHERE bs.slot_status IN ('CHECKED_IN', 'CHECK_OUT', 'NO_SHOW') " +
		    "  AND bs.booking_date >= CAST(DATEADD(MONTH, -1, GETDATE()) AS DATE) " +
		    "  AND bs.booking_date <= CAST(GETDATE() AS DATE) " +
		    "  AND ts.start_time   >= '05:00:00' " +
		    "  AND ts.start_time    < '22:00:00' " +
		    "GROUP BY " +
		    "    DATEPART(WEEKDAY, bs.booking_date), " +
		...
		private static final String SQL_CLASSIFIED =
		    ...
		    "          AND bs.slot_status  IN ('CHECKED_IN', 'CHECK_OUT', 'NO_SHOW') " +
		    "          AND bs.booking_date >= CAST(DATEADD(MONTH,-1,GETDATE()) AS DATE) " +
		    "          AND bs.booking_date <= CAST(GETDATE() AS DATE) " +
