...
<div class="main-content">
    ...
    <div class="dov-wrap">
        ...
        <div class="dov-card dov-fadein dov-d1">
            ...
            <%-- Form --%>
            <form id="ownerChangePwForm"
                  action="${pageContext.request.contextPath}/owner/setting/change-password"
                  method="post"
                  class="sof-form">
                ...
                <div class="sof-field">
                    ...
                    <div class="sof-input-wrap">
                        <input type="password" id="ownerOldPassword" name="oldPassword"
                               required maxlength="20"
                               class="sof-input"
                               placeholder="Nhập mật khẩu hiện tại"
                               oninput="ownerValidatePwLength(this)"
                               onblur="ownerValidatePwLength(this)">
                        <button type="button" class="sof-toggle-pw" onclick="ownerTogglePw(this)">
                            <i class="bi bi-eye-slash"></i>
                        </button>
                    </div>
                    <p class="sof-field-error" id="ownerOldPwError"></p>
                </div>
                ...
                <div class="sof-field">
                    ...
                    <div class="sof-input-wrap">
                        <input type="password" id="ownerNewPassword" name="newPassword"
                               required minlength="6" maxlength="20"
                               class="sof-input"
                               placeholder="Nhập mật khẩu mới (6–20 ký tự)"
                               oninput="ownerOnNewPwInput(this)"
                               onblur="ownerOnNewPwInput(this)">
                        <button type="button" class="sof-toggle-pw" onclick="ownerTogglePw(this)">
                            <i class="bi bi-eye-slash"></i>
                        </button>
                    </div>
                    <p class="sof-field-error" id="ownerNewPwError"></p>
                    <p id="ownerPasswordStrength" style="margin-top:.25rem; font-size:.75rem; font-weight:900; text-transform:uppercase; letter-spacing:.05em;"></p>
                    <p id="ownerPasswordSuggestion" style="font-size:.6875rem; font-weight:700; color:#9CA3AF; margin-top:.25rem;"></p>
                </div>
                ...
                <div class="sof-field">
                    ...
                    <div class="sof-input-wrap">
                        <input type="password" id="ownerConfirmPassword" name="confirmPassword"
                               required maxlength="20"
                               class="sof-input"
                               placeholder="Nhập lại mật khẩu mới"
                               oninput="ownerCheckConfirmInline()"
                               onblur="ownerCheckConfirmInline()">
                        <button type="button" class="sof-toggle-pw" onclick="ownerTogglePw(this)">
                            <i class="bi bi-eye-slash"></i>
                        </button>
                    </div>
                    <p class="sof-field-error" id="ownerConfirmPwError"></p>
                </div>

                <%-- Buttons --%>
                <div class="sof-actions">
                    <button type="button" class="sof-btn-save" onclick="ownerSubmitPwForm()">
...
<script>
    function ownerSetPwError(input, errId, msg) {
    ...
    function ownerValidatePwLength(input) {
        var errId = input.id === 'ownerOldPassword' ? 'ownerOldPwError' : 'ownerNewPwError';
        var val   = input.value;
        ownerClearPwError(input, errId);
        if (!val) return true;
        if (val.length < 6)  { ownerSetPwError(input, errId, 'Mật khẩu phải có ít nhất 6 ký tự'); return false; }
        if (val.length > 20) { ownerSetPwError(input, errId, 'Mật khẩu tối đa 20 ký tự'); return false; }
        ownerSetValid(input);
        return true;
    }
    function ownerCheckPasswordStrength(val) {
        var strengthEl   = document.getElementById('ownerPasswordStrength');
        var suggestionEl = document.getElementById('ownerPasswordSuggestion');
    ...
    function ownerOnNewPwInput(input) {
        ownerValidatePwLength(input);
        ownerCheckPasswordStrength(input.value);
        ownerCheckConfirmInline();
    }
    function ownerCheckConfirmInline() {
        var newPw     = document.getElementById('ownerNewPassword');
        var confirmPw = document.getElementById('ownerConfirmPassword');
        if (!confirmPw || !confirmPw.value) { ownerClearPwError(confirmPw, 'ownerConfirmPwError'); return true; }
        if (newPw && newPw.value !== confirmPw.value) {
            ownerSetPwError(confirmPw, 'ownerConfirmPwError', 'Mật khẩu xác nhận không khớp'); return false;
        }
        ownerClearPwError(confirmPw, 'ownerConfirmPwError');
        ownerSetValid(confirmPw);
        return true;
    }
    async function ownerSubmitPwForm() {
        var oldPw     = document.getElementById('ownerOldPassword');
        var newPw     = document.getElementById('ownerNewPassword');
        var confirmPw = document.getElementById('ownerConfirmPassword');
        var ok1 = ownerValidatePwLength(oldPw);
        var ok2 = ownerValidatePwLength(newPw);
        var ok3 = ownerCheckConfirmInline();
        if (!oldPw.value)     { ownerSetPwError(oldPw,     'ownerOldPwError',     'Vui lòng nhập mật khẩu hiện tại'); ok1 = false; }
        if (!newPw.value)     { ownerSetPwError(newPw,     'ownerNewPwError',     'Vui lòng nhập mật khẩu mới');      ok2 = false; }
        if (!confirmPw.value) { ownerSetPwError(confirmPw, 'ownerConfirmPwError', 'Vui lòng xác nhận mật khẩu');      ok3 = false; }
        ...
        if (result.isConfirmed) document.getElementById('ownerChangePwForm').submit();
    }
</script>
