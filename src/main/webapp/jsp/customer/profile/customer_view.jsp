...
<div class="flex flex-col min-h-screen bg-white relative">
	<div
		...
		<a href="${pageContext.request.contextPath}/home"
			...
			<i data-lucide="arrow-left" class="w-6 h-6"></i> <span> Quay
				Lại Trang Chủ</span>
		</a>
	</div>
	...
	<form action="customerController" method="POST"
		...
		<div class="space-y-4">
			<label
				...
				<i data-lucide="user-circle" class="w-4 h-4 text-emerald-700"></i> <span>Ảnh
					đại diện</span>
			</label>
			<div
				class="avatar-upload flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-6">
				<div
					class="avatar-preview w-24 h-24 rounded-full overflow-hidden border-2 border-gray-200 bg-gray-50 flex items-center justify-center">
					<c:choose>
				...
				<div class="flex flex-col space-y-2">
					<input type="file" id="avatar" name="avatar" accept="image/*"
						class="hidden" onchange="previewAvatar(this)"> <label
						for="avatar"
						class="btn-upload inline-flex items-center space-x-2 px-4 py-2 bg-emerald-600 text-white rounded-lg cursor-pointer hover:bg-emerald-700 transition-colors shadow-sm">
						<i data-lucide="camera" class="w-4 h-4"></i> <span>Chọn ảnh
							đại diện</span>
					</label>
		...
		<div class="space-y-2">
			<label for="full_name"
				class="text-sm font-medium text-gray-700 flex items-center space-x-2">
				<i data-lucide="user" class="w-4 h-4 text-emerald-700"></i> <span>Họ
					và tên</span>
		...
		<div class="space-y-2">
			<label for="email"
				class="text-sm font-medium text-gray-700 flex items-center space-x-2">
				<i data-lucide="mail" class="w-4 h-4 text-emerald-700"></i> <span>Email</span>
		...
		<div class="space-y-2">
			<label for="phone"
				class="text-sm font-medium text-gray-700 flex items-center space-x-2">
				<i data-lucide="phone" class="w-4 h-4 text-emerald-700"></i> <span>Số
					điện thoại</span> <span class="text-red-500">*</span>
			</label>
		...
		<!-- Buttons -->
		<div
			class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4 pt-4">
			<button type="submit"
			...
			<button type="reset"
				...
				<i data-lucide="refresh-cw" class="w-5 h-5"></i> <span>Khôi
					phục</span>
			</button>
		</div>
	</form>
	...
	<c:if test="${not empty sessionScope.updateSuccess}">
		<div class="notification success mx-6 mt-4">
			<i data-lucide="check-circle" class="w-5 h-5"></i> <span>${sessionScope.updateSuccess}</span>
		</div>
	...
	<c:if test="${not empty sessionScope.updateError}">
		<div class="notification error mx-6 mt-4">
			<i data-lucide="alert-circle" class="w-5 h-5"></i> <span>${sessionScope.updateError}</span>
		</div>
